...
| Immediate confirmation after the launcher closes                             | Dynamic Island     |
...
Use `confirmAlert()` for a blocking decision before a destructive or sensitive action. Confirmation dialogs are not replacements for Feedback Bar messages.
