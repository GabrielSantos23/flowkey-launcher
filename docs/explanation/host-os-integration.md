...
- **Locale & Internationalization (`crate::locale`):** Centralizes all BCP-47 and POSIX locale parsing, candidate resolution, and CLDR formatting inside Rust. It resolves localized application display names from macOS `InfoPlist.loctable` and Linux XDG `.desktop` entries using a **Language-First** hierarchy, while evaluating number formatting (decimal separators and grouping in the natural-language calculator) using a **Region-First** hierarchy. `LocaleService` is managed in Tauri state and dynamically refreshes when the host OS locale changes without requiring application restarts. See [Locale Subsystem & Internationalization](./locale-and-internationalization.md).
...
- **Window Management:** Asyar declares three windows in `tauri.conf.json`:
  ...
  - **`island`** (the Dynamic Island) is a small transparent always-on-top webview at `/island` used by the `islandService.show(...)` primitive (SDK namespace `island` for Tier 2 extensions). It is pre-declared with `visible: false` and is shown on demand by the `show_island` Tauri command, which positions it at the top-center of the monitor (via the `monitor` toolkit crate), emits an `island:show` event with `{icon, title, subtitle}`, and schedules an auto-hide via a tracked `tauri::async_runtime::JoinHandle` stored in `IslandState`. Lifecycle lives in `src-tauri/src/island_window/service.rs`; the Tauri command `commands/island.rs` is a thin wrapper. Because the island is its own window, it survives the main launcher window being hidden — that's the entire point of the primitive. The Windows SMTC media watcher (`src-tauri/src/media_watcher.rs`) feeds it track-change events.
