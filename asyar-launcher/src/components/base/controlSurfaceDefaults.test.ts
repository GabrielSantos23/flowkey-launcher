...
describe('shared control surface defaults', () => {
  ...
  it('keeps default badges visible inside SettingsCard surfaces', async () => {
    const css = await readSource('resources/styles/components.css');

    expect(cssRule(css, '.badge-default {')).toContain('background-color: var(--bg-secondary);');
  });

  it('keeps default selects visible inside SettingsCard surfaces', async () => {
    const css = await readSource('resources/styles/components.css');

    expect(cssRule(css, '.select-el {')).toContain('background-color: var(--bg-secondary);');
  });
});
