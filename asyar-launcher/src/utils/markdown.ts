...
renderer.code = function (token) {
  ...
  return (
    ...
    `<div class="md-code-header">${langLabel}<button type="button" class="md-copy-btn btn btn-secondary" data-code="${encodeURIComponent(code)}">${escapeHtml('Copy')}</button></div>` +
...
export async function handleMarkdownCopyClick(e: MouseEvent): Promise<void> {
  ...
  try {
    ...
    await feedbackService.report({
      ...
      severity: 'error',
      retryable: false,
      context: { message: 'Could not copy to clipboard. Try again.' },
    });
    return;
  }
  btn.disabled = true;
