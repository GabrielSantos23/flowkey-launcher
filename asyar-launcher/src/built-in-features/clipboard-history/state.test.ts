...
vi.mock('../../services/clipboard/stores/clipboardHistoryStore', () => ({
...
import { ClipboardViewStateClass } from './state';
...
describe('Type filtering', () => {
  ...
  it('returns all items when filter is "all"', () => {
    ...
    expect(state.filteredItems).toHaveLength(5);
  });
...
describe('showRenderedHtml state', () => {
  ...
  it('setting the selected item preserves showRenderedHtml (user preference persists)', () => {
    const items = [
...
describe('moveSelectionAndExtend', () => {
  ...
  beforeEach(() => {
    ...
    state.selectedItemId = state.filteredItems[0]?.id ?? null;
  });
