...
const mockStoreReload = vi.hoisted(() => vi.fn().mockResolvedValue(undefined));
const mockStoreInit = vi.hoisted(() => vi.fn().mockResolvedValue(undefined));
...
vi.mock('./shortcutStore', () => ({
  shortcutStore: {
    ...
    get shortcuts() {
      return mockStoreGetAll();
    },
    reload: mockStoreReload,
  },
}));
...
vi.mock('@tauri-apps/api/event', () => ({
  listen: vi.fn((event: string, handler: (event: { payload: string }) => void) => {
    ...
    return Promise.resolve(() => delete eventHandlers[event]);
  }),
}));
...
beforeEach(() => {
  ...
  mockPortalGetById.mockImplementation((id: string) => ({
    id,
    name: 'Portal',
...
describe('init', () => {
  it('does nothing when the store is empty', async () => {
    ...
    expect(mockStoreInit).toHaveBeenCalledTimes(1);
    expect(mockGetValidShortcutKeys).toHaveBeenCalledTimes(1);
    expect(mockRegisterItemShortcut).not.toHaveBeenCalled();
  });

  it('reloads the store once and retries when an objectId is unknown (self-heal)', async () => {
    // First lookup misses (stale cache), the post-reload lookup hits.
    let calls = 0;
    mockStoreGetByObjectId.mockImplementation(() => {
      calls += 1;
      return calls === 1 ? undefined : makeShortcut({ itemType: 'command', objectId: 'cmd_late' });
    });
    mockStoreGetAll.mockReturnValue([makeShortcut({ itemType: 'command', objectId: 'cmd_late' })]);

    await shortcutService.handleFiredShortcut('cmd_late');

    expect(mockStoreReload).toHaveBeenCalled();
    expect(mockWithReplacementSemantics).toHaveBeenCalled();
  });

  it('gives up after one reload when the objectId is genuinely unknown', async () => {
    mockStoreGetByObjectId.mockReturnValue(undefined);

    await shortcutService.handleFiredShortcut('cmd_unknown');

    expect(mockStoreReload).toHaveBeenCalledTimes(1);
    expect(mockWithReplacementSemantics).not.toHaveBeenCalled();
  });

  it('subscribes to user-shortcut-fired and dispatches the object id', async () => {
    mockStoreGetByObjectId.mockReturnValue(
      makeShortcut({ itemType: 'command', objectId: 'cmd_x' }),
    );
    await shortcutService.init();
    expect(eventHandlers['user-shortcut-fired']).toBeDefined();

    await eventHandlers['user-shortcut-fired']({ payload: 'cmd_x' });
    expect(mockWithReplacementSemantics).toHaveBeenCalled();
  });
...
describe('isConflict', () => {
  ...
  it('returns the conflicting item when the shortcut is already taken', async () => {
    mockStoreGetAll.mockReturnValue([
      makeShortcut({ shortcut: 'Alt+A', objectId: 'obj-1', itemName: 'App' }),
    ]);
...
describe('register', () => {
  it('returns a conflict when the shortcut is already taken by another item', async () => {
    mockStoreGetAll.mockReturnValue([
      makeShortcut({ shortcut: 'Alt+A', objectId: 'other', itemName: 'Other' }),
    ]);
...
describe('unregister', () => {
  ...
  it('invokes unregister_item_shortcut with the parsed modifier and key', async () => {
    mockStoreGetByObjectId.mockReturnValue(
      makeShortcut({ shortcut: 'Control+J', objectId: 'obj-1' }),
    );
  ...
  it('opens an application when itemType is "application"', async () => {
    mockStoreGetByObjectId.mockReturnValue(
      makeShortcut({
        ...
        itemName: 'Finder',
        itemType: 'application',
        itemPath: '/App/Finder.app',
      }),
    );
    await shortcutService.handleFiredShortcut('obj-1');
    expect(mockAppOpen).toHaveBeenCalledWith(
      expect.objectContaining({
        objectId: 'obj-1',
        name: 'Finder',
        path: '/App/Finder.app',
