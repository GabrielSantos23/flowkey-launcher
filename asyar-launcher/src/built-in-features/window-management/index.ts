...
class WindowManagementExtension implements Extension {
  ...
  private async applyLayoutByArgs(args?: Record<string, any>): Promise<void> {
    ...
    if (!rawName && !rawId) {
      await islandService.show({
        icon: '🪟',
        title: 'No layout name or ID provided',
    ...
    if (layout) {
      ...
      await islandService.show({
        icon: '🪟',
        title: `Layout "${missing}" not found`,
        dismissLauncher: true,
      });
      await feedbackService.report({
        source: 'frontend',
  ...
  private async applyPreset(presetId: string): Promise<void> {
    try {
      ...
      const label = presetId
        ...
        .join(' ');
      await islandService.show({ icon: '🖥️', title: label, dismissLauncher: true });
    } catch (err: any) {
  ...
  private async restorePreviousBounds(): Promise<void> {
    ...
    try {
      ...
      await islandService.show({ icon: '🪟', title: 'Restored', dismissLauncher: true });
    } catch (err: any) {
      await feedbackService.report({
        source: 'frontend',
        kind: 'manual',
  ...
  private async saveCurrentWindowLayout(): Promise<void> {
    ...
    try {
      ...
      await islandService.show({ icon: '🪟', title: `Saved "${name}"`, dismissLauncher: true });
    } catch (err: any) {
      await feedbackService.report({
        source: 'frontend',
        kind: 'manual',
