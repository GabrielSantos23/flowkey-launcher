...
export class RunService {
   ...
   * UNACK_FAILED_CAP.
   */
  unacknowledgedScriptResults: Run[] = [];
  ...
  constructor() {
    // Fire-and-forget: a rejection (e.g. plain node test env, no Tauri window)
    // must not surface as an unhandled rejection that fails the test run.
    void this.subscribe().catch((err) =>
      logService?.warn(`[RunService] event subscribe failed: ${err}`),
    );
  }
