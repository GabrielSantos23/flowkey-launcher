...
class ViewManagerClass {
  #activeView = new Signal<string | null>(null);
  #activeViewSearchable = new Signal(false);
  #activeViewPlaceholder = new Signal<string | null>(null);
  get activeView(): string | null {
    return this.#activeView.get();
  }
  set activeView(v: string | null) {
    this.#activeView.set(v);
  }
  get activeViewSearchable(): boolean {
    return this.#activeViewSearchable.get();
  }
  set activeViewSearchable(v: boolean) {
    this.#activeViewSearchable.set(v);
  }
  get activeViewPlaceholder(): string | null {
    return this.#activeViewPlaceholder.get();
  }
  set activeViewPlaceholder(v: string | null) {
    this.#activeViewPlaceholder.set(v);
  }
  activeViewPrimaryActionLabel: string | null = null;
  activeViewSubtitle: string | null = null;
  ...
  navigateToView(viewPath: string): void {
    ...
    if (
      ...
      logService.debug(
        `[ViewManager] navigateToView: skipping duplicate push for ${viewPath} (already top of stack)`,
      );
      return;
    }
    ...
    const viewName = viewPath.split('/')[1];
    const command = manifest.commands?.find((c) => c.component === viewName || c.id === viewName);
    const placeholder = command?.searchBarPlaceholder ?? null;

    const newState: NavigationState = {
      viewPath,
      searchable: (command as any)?.searchable ?? manifest.searchable ?? false,
    ...
    this.activeViewPlaceholder = newState.placeholder ?? null;
    ...
    logService.debug(
      `Navigated to view: ${viewPath}, searchable: ${newState.searchable}. Stack size: ${this.navigationStack.length}`,
    );
  }
  ...
  goBack(): void {
    ...
    logService.debug(
      `Going back from view: ${closedState.viewPath}. Stack size after pop: ${this.navigationStack.length}`,
    );
    ...
    if (this.navigationStack.length === 0) {
      ...
      this.activeView = null;
      this.activeViewSearchable = false;
      this.activeViewPlaceholder = null;
      ...
      if (module) {
        ...
        if (extensionInstance && typeof extensionInstance.onViewSearch === 'function') {
          try {
            ...
            logService.error(
              `[ViewManager] Error calling onViewSearch for ${extensionId}: ${error}`,
            );
          }
        }
      } else {
      ...
      if (module) {
        ...
        if (extensionInstance && typeof extensionInstance.onViewSubmit === 'function') {
          try {
            ...
            logService.error(
              `[ViewManager] Error calling onViewSubmit for ${extensionId}: ${error}`,
            );
          }
          return;
        }
