...
vi.mock('./preferencesPromptStore', () => ({
  preferencesPromptStore: { open: vi.fn() },
}));
import { preferencesPromptStore } from './preferencesPromptStore';
...
describe('preference gate bypass', () => {
  ...
  it('enforces preference gate when no bypass flag is set', async () => {
    ...
    await expect(svc.executeCommand('cmd_ext_normal')).rejects.toThrow('requires preferences');
...
describe('updateCommandMetadata', () => {
  ...
  it('throws when command is not registered', async () => {
    ...
    await expect(svc.updateCommandMetadata('unknown-ext', 'missing', 'test')).rejects.toThrow(
      "Command 'missing' not found for extension 'unknown-ext'",
    );
  });
  ...
  it('throws when extension does not own the command', async () => {
    ...
    await expect(svc.updateCommandMetadata('ext-b', 'do-thing', 'hack')).rejects.toThrow(
      "Command 'do-thing' not found for extension 'ext-b'",
    );
  });
});
