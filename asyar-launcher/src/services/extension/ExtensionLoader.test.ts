...
import { permissionConsentService } from './permissionConsentService';
...
vi.mock('./extensionDispatcher', () => ({ dispatch: vi.fn() }));
...
describe('ExtensionLoader Tier 2 no-view handler routes through dispatcher', () => {
  ...
  it('execute() calls dispatch with source=search and the correct payload', async () => {
    const { dispatch } = await import('./extensionDispatcher');
