...
class ExtensionPreferencesService {
  ...
  /**
   * Self-heal: register declarations from the Rust-side manifest when this
   * webview hasn't run extension discovery yet (e.g. the settings window
   * right after a reload). No-op when already registered or when the
   * extension is unknown to discovery.
   */
  async ensureRegistered(extensionId: string): Promise<void> {
    if (this.declarations.has(extensionId)) return;
    const record = await getExtension(extensionId);
    if (!record?.manifest) return;
    const manifest = record.manifest;
    this.registerManifest(extensionId, {
      ...
      commands: (manifest.commands ?? []).reduce(
        ...
        {} as Record<string, PreferenceDeclaration[]>,
      ),
    });
  }
  ...
  async getEffectivePreferences(extensionId: string): Promise<PreferenceBundle> {
    ...
    await this.ensureRegistered(extensionId);
