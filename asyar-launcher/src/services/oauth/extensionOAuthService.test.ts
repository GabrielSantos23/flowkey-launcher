...
import { ExtensionOAuthService } from './extensionOAuthService';
...
describe('ExtensionOAuthService', () => {
  ...
  describe('_handleCallback()', () => {
    ...
    it('ignores states it did not start (host-side flows own their exchange)', async () => {
      const { oauthExchangeCode } = await import('../../lib/ipc/commands');
      setMockIframes(1);

      const svc = new ExtensionOAuthService();
      // No authorize() was called — the state is unknown to this service.
      await svc._handleCallback('asyar://oauth/callback?code=xyz&state=unknown-state');

      expect(oauthExchangeCode).not.toHaveBeenCalled();
