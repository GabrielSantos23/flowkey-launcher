...
export class ExtensionOAuthService {
  ...
  async _handleCallback(url: string): Promise<void> {
    ...
    // Only handle flows this service started. Host-side flows (e.g. the
    // built-in Spotify feature) register their own deep-link listener and
    // own their exchange — exchanging here too would race them for the
    // single-use pending flow in Rust.
    if (!this._pendingFlows.has(state)) return;
