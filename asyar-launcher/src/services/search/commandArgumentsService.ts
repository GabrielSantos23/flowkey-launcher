...
export class CommandArgumentsService {
  private _activeSignal = new Signal<ActiveArgumentMode | null>(null);

  /** Reactive read for all legacy `this._active` call sites. */
  private get _active(): ActiveArgumentMode | null {
    return this._activeSignal.get();
  }
   ...
   */
  private _blockedNotice: string | null = null;
  ...
  set active(v: ActiveArgumentMode | null) {
    this._activeSignal.set(v);
  }
  ...
  private applyResolved(
    ...
    this._activeSignal.set({ ...this._active, resolved });
  }
  ...
  async submit(): Promise<void> {
    ...
    if (this._active === active) this._activeSignal.set(null);
  }
}
