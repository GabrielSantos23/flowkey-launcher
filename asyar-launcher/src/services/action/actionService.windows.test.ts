...
vi.mock('../search/searchOrchestrator', () => ({
  searchOrchestrator: mockSearchOrchestrator,
}));

const mockSearchStores = vi.hoisted(() => ({ selectedIndex: -1 }));
vi.mock('../search/stores/search', () => ({ searchStores: mockSearchStores }));

const mockFeedbackService = vi.hoisted(() => ({
  confirmAlert: vi.fn().mockResolvedValue(true),
}));

const mockIslandService = vi.hoisted(() => ({
  show: vi.fn().mockResolvedValue(undefined),
}));
vi.mock('../island/islandService', () => ({
  islandService: mockIslandService,
}));
...
function makeLnkResult(overrides: Partial<{ path: string; name: string }> = {}) {
  return {
    ...
    score: 1,
    path:
      overrides.path ?? 'C:\\ProgramData\\Microsoft\\Windows\\Start Menu\\Programs\\Firefox.lnk',
  };
}
...
describe('uninstall_application on Windows', () => {
  beforeEach(() => {
    ...
    mockIslandService.show.mockReset().mockResolvedValue(undefined);
  ...
  it('execute calls uninstallApplication with the .lnk path and empty dataPaths on confirm', async () => {
    ...
    expect(mockIslandService.show).toHaveBeenCalledWith(
      expect.objectContaining({ title: 'Uninstaller launched' }),
    );
  });
  ...
  it('execute surfaces a failure HUD when Rust rejects', async () => {
    ...
    expect(mockIslandService.show).toHaveBeenCalledOnce();
    const islandArg = mockIslandService.show.mock.calls[0][0];
    expect(islandArg.title).toMatch(/Uninstall failed/);
    expect(islandArg.title).toMatch(/no uninstall registry entry/i);
  });
  ...
  it('execute does not call uninstall when user cancels', async () => {
    ...
    expect(mockIslandService.show).not.toHaveBeenCalled();
  });
});
