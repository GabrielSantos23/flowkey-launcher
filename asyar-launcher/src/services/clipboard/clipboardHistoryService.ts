...
import { clipboardHistoryStore } from './stores/clipboardHistoryStore';
...
export class ClipboardHistoryService implements IClipboardHistoryService {
  ...
  private lastImageCapture: { value: string; timestamp: number } | null = null;
  ...
  private async startMonitoring(): Promise<void> {
    ...
    this.unlistenClipboard = await onClipboardChange((result: ReadClipboard) => {
      this.clipboardQueue = this.clipboardQueue
        .then(() => this.handleClipboardChange(result))
        .catch((err) => {
          logService.error(`Clipboard change handler failed: ${err}`);
        });
    });
  ...
  private async captureTextContent(text: string, sourceApp?: ClipboardSourceApp): Promise<void> {
    try {
      ...
      await this.notifyCaptured('📄', 'Copied', item.preview, sourceApp);
    } catch (error) {
  ...
  private async captureImageContent(
    ...
    try {
      ...
      const now = Date.now();
      if (
        this.lastImageCapture &&
        this.lastImageCapture.value === imageData.value &&
        now - this.lastImageCapture.timestamp < 1000
      ) {
        logService.debug(
          `Ignoring duplicate image capture within debounce window: ${imageData.value}`,
        );
        return;
      }
      this.lastImageCapture = { value: imageData.value, timestamp: now };
      ...
      await clipboardHistoryStore.addHistoryItem(item);
      await this.notifyCaptured('🖼️', 'Image copied', item.preview, sourceApp);
    } catch (error) {
  ...
  private async captureFileContent(
    ...
    try {
      ...
      await clipboardHistoryStore.addHistoryItem(item);
      await this.notifyCaptured('📂', 'Files copied', item.preview, sourceApp);
    } catch (error) {
  ...
  /**
   * Dynamic Island notification for a system-wide copy. Suppressed when the
   * copy originates from Asyar itself — flows that copy-and-notify (deeplink
   * copy, Spotify track URL) already show their own island, and a second
   * "Copied" pill a second later is noise.
   */
  private async notifyCaptured(
    icon: string,
    title: string,
    preview: string,
    sourceApp?: ClipboardSourceApp,
  ): Promise<void> {
    const source = sourceApp?.name?.toLowerCase() ?? '';
    if (source.includes('asyar') || source.includes('flowkey')) return;
    try {
      await islandService.show({ icon, title, subtitle: preview });
    } catch (error) {
      // The island is garnish; a failure must never break capture.
      logService.debug(`Copied island failed: ${error}`);
    }
  }
    ...
    try {
      ...
      if (!(await commands.checkAccessibilityPermission())) {
        ...
        void feedbackService.report({
          ...
          context: {
            message:
              'Flowkey needs macOS Accessibility permission to paste. Enable Flowkey under ' +
