...
export const applicationIndexService = {
  async subscribe(caller: string | null, eventTypes: string[]): Promise<string | null> {
    return applicationIndexSubscribe(caller, eventTypes);
  },

  async unsubscribe(caller: string | null, subscriptionId: string): Promise<void> {
    await applicationIndexUnsubscribe(caller, subscriptionId);
  },
};
