...
class InspectorStore {
  isOpen = false;
  selectedExtensionId: string | null = null;
  activeTab: 'runtime' | 'state' | 'subscriptions' | 'events' | 'rpc' | 'ipc' | 'help' = 'runtime';

  runtimeMap: Record<string, RuntimeEntry> = {};
  stateByExt: Record<string, StateEntry[]> = {};
  subsByExt: Record<string, SubscriptionSummary[]> = {};
  eventsByExt: Record<string, EventRow[]> = {};
  #eventSeq = 0;

  rpcByExt: Record<string, Record<string, RpcTrace>> = {};
  ipcByExt: Record<string, IpcTrace[]> = {};
  #ipcSeq = 0;
