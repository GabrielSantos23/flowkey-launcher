...
export async function oauthRefreshToken(params: {
  ...
  return invokeSafe<OAuthTokenPayload | null>('oauth_refresh_token', params);
}

export async function oauthGetStoredTokenRefreshable(
  extensionId: string,
  providerId: string,
): Promise<OAuthTokenPayload | null> {
  return invokeSafe<OAuthTokenPayload | null>('oauth_get_stored_token_refreshable', {
    extensionId,
    providerId,
  });
}
