...
import { bumpArgumentHintVersion } from '../launcher/argumentHintVersion';
...
/** Read-only look at an unpacked extension folder (manifest + compatibility). */
export async function inspectExtensionFolder(
  path: string,
): Promise<ExtensionFolderInspection | null> {
  return invokeSafe<ExtensionFolderInspection>('inspect_extension_folder', { path });
}

export interface ExtensionFolderInspection {
  manifest: {
    id: string;
    name: string;
    version: string;
...
export async function showOpenFolderDialog(): Promise<string | null> {
  return invokeSafe<string | null>('show_open_folder_dialog');
}
