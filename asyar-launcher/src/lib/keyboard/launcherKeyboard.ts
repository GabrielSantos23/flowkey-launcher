...
export function createKeyboardHandlers(deps: KeyboardDeps) {
  ...
  // Cmd/Ctrl+Q: block quit — users quit via the "Quit Flowkey" command
  ...
  function tryToggleActionPanel(event: KeyboardEvent): boolean {
    ...
    const wasOpen = deps.getBottomBar()?.isOpen() ?? false;
    deps.getBottomBar()?.toggleActionList();
    // Closing the panel with Ctrl+K must hand focus back to the search bar —
    // the popup's own input unmounts and would otherwise leave focus on <body>.
    if (wasOpen) restoreSearchFocus({ select: true });
    return true;
  }
  ...
  function handleKeydown(event: KeyboardEvent) {
    if (shortcutStore.isCapturing || isAnyModalOpen(document)) {
      logService.debug(
        `[DIAG] key ${event.key} swallowed: capturing=${shortcutStore.isCapturing} modal=${isAnyModalOpen(document)}`,
      );
      return;
    }
    if (event.defaultPrevented) {
      logService.debug(`[DIAG] key ${event.key} swallowed: defaultPrevented`);
      return;
    }
