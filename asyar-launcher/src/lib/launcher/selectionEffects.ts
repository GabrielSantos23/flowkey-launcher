...
import { warmIfTier2 } from '../../services/search/searchOrchestrator';
...
export function setupSelectionEffects(state: LauncherState) {
  ...
  let runTierMap = new Map<string, number>();
  launcherEffect(() => {
  ...
  const remap = () => {
    try {
      console.log(
        'DBG remap: searchItems',
        state.searchItems?.length,
        'error?',
        !!state.searchItems,
      );
      const { mappedItems, selectedOriginal } = buildMappedItems({
        searchItems: state.searchItems as unknown as SearchResult[],
      ...
      console.log('DBG remap: write', mappedItems.length);
      state.searchResultItemsMapped = mappedItems;
      state.currentSelectedItemOriginal = selectedOriginal;
    } catch (err) {
      // Never let a mapping failure kill the pipeline silently.
      logService.error(`Failed to map search results: ${err}`);
    }
  };
  // The mapping must re-run when items arrive — including when they arrive
  // asynchronously from `handleSearch`, which signal tracking does not
  // survive (the read happens after an await). The orchestrator's explicit
  // listener guarantees it.
  searchOrchestrator.onItemsChanged(remap);
  launcherEffect(remap);
  ...
  launcherEffect(() => {
    ...
    if (item && (item.type === 'application' || item.type === 'command')) {
      ...
      actionService.registerAction({
        ...
        context: ActionContext.CORE,
        execute: async () => {
          state.assignAliasTarget = item as unknown as {
            objectId: string;
            name?: string;
            type?: string;
            alias?: string;
