...
export function useListSelection<T>({
  ...
  // Recomputed on every read — this replaced a Svelte `$derived`, which
  // re-evaluated lazily; a one-shot value would go stale as the list changes.
  const clamped = () => {
    const len = items().length;
    if (len === 0) {
      // Empty list resets the raw index so a refill starts at the top.
      raw = -1;
      return -1;
    }
