...
/// Refresh a stored token via the OAuth refresh-token grant. Requires the
/// stored token to carry a refresh token; the refreshed set is persisted and
/// returned. Spotify omits `refresh_token` in refresh responses — the
/// service layer preserves the previous one in that case.
#[tauri::command]
pub async fn oauth_refresh_token(
    ...
    let conn = data_store.conn()?;
    let stored = token_store::get_token(&app, &conn, &extension_id, &provider_id)?
        .ok_or_else(|| AppError::NotFound("No stored token to refresh".into()))?;

    let refresh_token = stored.refresh_token.as_deref().ok_or_else(|| {
        AppError::OAuth("Stored token has no refresh token — re-authorize instead".into())
    })?;

    let refreshed = service::refresh_token(&token_url, &client_id, refresh_token).await?;
    token_store::store_token(&app, &conn, &extension_id, &provider_id, &refreshed)?;
    Ok(refreshed)
}

/// Return the stored token for the given extension + provider EVEN IF its
/// access token is expired, as long as it carries a refresh token. Used by
/// host-side features (e.g. Spotify) that can silently refresh on startup —
/// discarding expired-but-refreshable tokens would force a full re-login
/// after every app restart.
#[tauri::command]
pub fn oauth_get_stored_token_refreshable(
    app: AppHandle,
    extension_id: String,
    provider_id: String,
    data_store: State<'_, DataStore>,
    ...
    let conn = data_store.conn()?;
    token_store::get_token(&app, &conn, &extension_id, &provider_id)
}
