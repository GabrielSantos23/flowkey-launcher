...
/// in input order. Shares platform semantics with
/// `extensions::discovery::validate_compatibility`, used for locally-installed
/// extensions.
#[tauri::command]
...
pub async fn show_open_extension_dialog(app_handle: AppHandle) -> Result<Option<String>, AppError> {
    ...
    let result = app_handle
        .dialog()
        .file()
        .add_filter("Flowkey Package", &["flowkey"])
        .blocking_pick_file();
    Ok(result.map(|p| p.to_string()))
}

/// A read-only look at an unpacked extension folder: manifest plus platform /
/// SDK compatibility. The extensions manager shows this before registering the
/// folder so the user can review name and permissions. Does NOT register
/// anything — that is `register_dev_extension`'s job.
#[tauri::command]
pub fn inspect_extension_folder(
    path: String,
) -> Result<extensions::discovery::FolderInspection, AppError> {
    extensions::discovery::inspect_folder(std::path::Path::new(&path))
}

/// Native folder picker for the "Install from Folder…" flow.
#[tauri::command]
pub async fn show_open_folder_dialog(app_handle: AppHandle) -> Result<Option<String>, AppError> {
    use tauri_plugin_dialog::DialogExt;
