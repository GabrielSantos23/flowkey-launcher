...
//! event enum and the Windows watcher/presence-query wiring.
...
/// for Windows; a fake is available for unit tests.
...
mod tests {
    ...
    fn wire_format_serializes_launched_with_camelcase_fields() {
        let ev = AppEvent::Launched {
            ...
            name: "App".into(),
            path: Some("C:\\Program Files\\App\\App.exe".into()),
        };
        let json = serde_json::to_value(&ev).unwrap();
        assert_eq!(json["type"], "launched");
        assert_eq!(json["pid"], 1234);
        assert_eq!(json["bundleId"], "com.example.app");
        assert_eq!(json["name"], "App");
        assert_eq!(json["path"], "C:\\Program Files\\App\\App.exe");
    }
