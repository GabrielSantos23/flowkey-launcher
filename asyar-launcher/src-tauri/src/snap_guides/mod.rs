...
//! a snap point.
//!
//! Modeled directly on [`crate::island_window`]: a pre-declared Tauri window
...
///
/// Deliberately **not** `specta::Type` — like [`crate::island_window::IslandContent`],
...
/// `windowCommands.ts`, same as `IslandContent`.
...
/// `snap-guides:state` event — the same race `IslandState` guards against.
#[derive(Default)]
