        ...
        Some(resolved_path) => {
            ...
            let content = match file_path {
                "view.html" => {
                    ...
                    let with_base_style = inject_view_base_style(with_role);
                    let content = maybe_inject_dev_inspector_flag(with_base_style);
                    match crate::ext_overrides::override_css_for(extension_id) {
                        Some(css) => crate::ext_overrides::inject_ext_override_css(content, css),
                        None => content,
                    }
                }
                "worker.html" => {
            ...
            // Entry documents and their scripts must revalidate: bundles are
            // rebuilt in place during extension development (entry filenames
            // are unhashed), and a stale cached view.js would keep referencing
            // removed content-hashed chunks.
            let cache_control = match canonical_path.extension().and_then(|e| e.to_str()) {
                Some("html") | Some("js") | Some("css") => "no-cache",
                _ => "max-age=3600",
            };
...
/// Injects a base stylesheet into `view.html` so every extension view
/// renders on the themed surface instead of the WebKit default canvas.
///
/// Extension bundles are authored against the SDK's CSS tokens, but nothing
/// in a bundle guarantees the document itself is painted: older bundles
/// carry no token block at all, and until the host posts
/// `asyar:theme:variables` the custom properties are undefined, so the
/// canvas falls back to the light color-scheme default (white) and
/// light-on-dark text becomes unreadable. The injected rule paints the
/// document with the opaque popup surface — NOT `--bg-primary`, whose dark
/// value is semi-transparent for the launcher's acrylic window and would
/// composite over the iframe's opaque white canvas as washed-out gray — and
/// opts the document into both color schemes so it follows the OS appearance
/// while the tokens load. The `var()` fallbacks are the dark-theme values,
/// not typos: tokens are genuinely absent in older bundles before the host
/// message arrives.
///
/// Inserted immediately after the opening `<head>` tag so any stylesheet the
/// bundle ships later in the head still wins. View documents only — workers
/// never paint.
pub(crate) fn inject_view_base_style(html: Vec<u8>) -> Vec<u8> {
    const BASE_STYLE: &str = "<style id=\"asyar-base-style\">html{color-scheme:light dark}html,body{margin:0;padding:0;background:var(--bg-popup,rgb(30,30,32));color:var(--text-primary,rgba(237,240,252,.96))}</style>";
    ...
    if let Some(pos) = lower.find("<head>") {
        let insert_at = pos + "<head>".len();
        let mut result = String::with_capacity(html_str.len() + BASE_STYLE.len());
        result.push_str(&html_str[..insert_at]);
        result.push_str(BASE_STYLE);
        result.push_str(&html_str[insert_at..]);
        result.into_bytes()
    } else {
        // No <head> — prepend the style wrapped in <head>.
        let mut result = String::with_capacity(html_str.len() + BASE_STYLE.len() + 15);
        result.push_str("<head>");
        result.push_str(BASE_STYLE);
        result.push_str("</head>");
        result.push_str(&html_str);
...
mod role_injection_tests {
    use super::{inject_asyar_role, inject_view_base_style};

    #[test]
    fn base_style_injected_into_view_html() {
        let html = b"<html><head><title>T</title></head><body></body></html>";
        let result = inject_view_base_style(html.to_vec());
        let s = String::from_utf8(result).unwrap();
        assert!(
            s.contains("<style id=\"asyar-base-style\">"),
            "base style must be injected, got: {}",
            s
        );
        assert!(
            s.contains("var(--bg-popup,"),
            "base style must paint the document with the themed surface"
        );
        let style_pos = s.find("asyar-base-style").expect("style must be present");
        let head_end = s.find("</head>").expect("</head> must be present");
        assert!(
            style_pos < head_end,
            "base style must appear before </head>"
        );
    }

    #[test]
    fn base_style_preceded_when_head_absent() {
        let html = b"<html><body>content</body></html>";
        let result = inject_view_base_style(html.to_vec());
        let s = String::from_utf8(result).unwrap();
        assert!(
            s.starts_with("<head>"),
            "base style must be prepended in a new <head>, got: {}",
            s
        );
        assert!(s.contains("asyar-base-style"));
    }

    #[test]
    fn base_style_and_role_script_both_inside_head() {
        let html = b"<html><head></head><body>view</body></html>";
        let with_role = inject_asyar_role(html, "view");
        let result = inject_view_base_style(with_role);
        let s = String::from_utf8(result).unwrap();
        let head_end = s.find("</head>").expect("</head> must be present");
        let role_pos = s.find("__ASYAR_ROLE__").expect("role script present");
        let style_pos = s.find("asyar-base-style").expect("style present");
        assert!(role_pos < head_end && style_pos < head_end);
    }
