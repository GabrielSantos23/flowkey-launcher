...
pub async fn fetch(req: FetchRequest) -> Result<serde_json::Value, AppError> {
    ...
    // The SDK contract (`NetworkResponse.body`) promises text bodies verbatim
    // and binary bodies base64-encoded. `response.text()` would lossily
    // coerce binary bytes (images, archives) into mojibake, so binary
    // content types are read as raw bytes and base64-encoded instead.
...
mod tests {
    ...
    #[tokio::test]
    async fn base64_encodes_binary_response_bodies() {
        let mut server = Server::new_async().await;
        let png: Vec<u8> = vec![0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A, 0xFF, 0xD8];
        let clone = png.clone();
        let mock = server
            .mock("GET", "/img")
        ...
        let res = fetch(req(&format!("{}/img", server.url()))).await.unwrap();
        mock.assert();
