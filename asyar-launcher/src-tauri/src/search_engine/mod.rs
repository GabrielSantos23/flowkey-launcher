...
fn init_db(conn: &rusqlite::Connection) -> Result<(), SearchError> {
    ...
    favorites::init_table(conn)
        .map_err(|e| SearchError::Other(format!("Failed to initialize favorites table: {}", e)))?;
    Ok(())
}
...
impl SearchState {
    ...
    /// Toggle the user favorite pin on a search-index object id. Returns the
    /// resulting state: `true` = favorited, `false` = unfavorited. The pin is
    /// stored regardless of whether the id is currently indexed; rendering
    /// skips pins that resolve to no item, so pins survive temporary
    /// unavailability of the target (e.g. an extension that failed to load).
    pub fn favorite_toggle(&self, object_id: &str) -> Result<bool, SearchError> {
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
            .as_secs() as i64;
        let conn = self.db.lock().map_err(|_| SearchError::LockError)?;
        favorites::toggle(&conn, object_id, now)
            .map_err(|e| SearchError::Other(format!("favorite_toggle failed: {}", e)))
    }

    /// Favorited object ids, newest-pinned first.
    pub fn favorites_list(&self) -> Result<Vec<String>, SearchError> {
        let conn = self.db.lock().map_err(|_| SearchError::LockError)?;
        favorites::list_ids(&conn)
            .map_err(|e| SearchError::Other(format!("favorites_list failed: {}", e)))
    }

    /// Resolve favorite pins into displayable results. Pins whose id is no
    /// longer in the index (uninstalled app, removed extension command) are
    /// skipped, not dropped — re-indexing the item brings its favorite back.
    fn favorite_results(&self) -> Result<Vec<SearchResult>, SearchError> {
        let ids = self.favorites_list()?;
        let guard = self.items.read().map_err(|_| SearchError::LockError)?;
        let mut results = Vec::with_capacity(ids.len());
        for id in ids {
            if let Some(item) = guard.iter().find(|item| item.id() == id) {
                results.push(SearchResult {
                    ...
                    path: None,
                    icon: match item {
                        SearchableItem::Application(a) => a.icon.clone(),
                        SearchableItem::Command(c) => c.icon.clone(),
                    },
                    extension_id: match item {
                        SearchableItem::Application(_) => None,
                        SearchableItem::Command(c) => Some(c.extension.clone()),
                    },
    ...
    pub fn merged_search(
        ...
        if query.trim().is_empty() {
            ...
            let mut ordered = self.favorite_results()?;
            ordered.append(&mut pinned);
...
mod service_tests {
    ...
    fn favorite_toggle_round_trips() {
        let state = make_state();
        assert!(state.favorite_toggle("app_safari").unwrap());
        assert_eq!(state.favorites_list().unwrap(), vec!["app_safari"]);
        assert!(!state.favorite_toggle("app_safari").unwrap());
        assert!(state.favorites_list().unwrap().is_empty());
    }

    #[test]
    fn merged_search_empty_query_leads_with_favorites() {
        let state = make_state();
        state.index_one(app("app_safari", "Safari", 0)).unwrap();
        state.index_one(app("app_notes", "Notes", 99)).unwrap();
        state.favorite_toggle("app_safari").unwrap();

        let results = state.merged_search("", vec![], 10, &[]).unwrap();
        assert_eq!(results[0].object_id, "app_safari");
        assert_eq!(results[0].tier, ranker::Tier::Pinned as u8);
        // Favorited rows must not duplicate when the frecency list also
        // contains them.
        assert_eq!(
            results
                .iter()
        ...
        // The non-favorited item still follows.
        assert!(results.iter().any(|r| r.object_id == "app_notes"));
    }

    #[test]
    fn merged_search_skips_favorites_missing_from_index() {
        let state = make_state();
        state.favorite_toggle("app_ghost").unwrap();
        let results = state.merged_search("", vec![], 10, &[]).unwrap();
        assert!(results.iter().all(|r| r.object_id != "app_ghost"));
    }
