//! Extension installation: verify and extract extension packages.

use crate::error::AppError;
use crate::extensions::{read_theme_definition, ExtensionManifest};
...
/// Verify a file's SHA-256 against a `sha256:<hex>` expectation. Used by the
/// runtime downloader; local `.flowkey` installs carry no checksum by design.
...
/// Validate that a file path exists and has .flowkey extension.
pub(crate) fn validate_file_path(path: &Path) -> Result<(), AppError> {
    ...
    match path.extension().and_then(|e| e.to_str()) {
        ...
        _ => Err(AppError::Validation(
            "File must have .flowkey extension".to_string(),
        )),
    }
}

/// Install an extension from a local .flowkey file.
...
mod tests {
    ...
    #[tokio::test]
    async fn validate_file_path_rejects_non_flowkey() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains(".flowkey"));
        let _ = std::fs::remove_file(&bad_path);
    }

    #[tokio::test]
    async fn validate_file_path_rejects_nonexistent() {
        let result = validate_file_path(Path::new("/nonexistent/file.flowkey"));
        assert!(result.is_err());
    }
