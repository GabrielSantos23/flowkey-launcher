...
/// Result of a read-only look at an unpacked extension folder: the parsed
/// manifest plus its platform / SDK compatibility verdict. Returned by
/// `inspect_extension_folder` so the extensions manager can show name and
/// permissions before the user commits to registering the folder.
...
/// Read-only inspection of an unpacked extension folder. Validates
/// `manifest.json` (at the folder root or under `dist/`) and evaluates
/// compatibility, but registers nothing.
pub fn inspect_folder(path: &Path) -> Result<FolderInspection, AppError> {
    let manifest_path = path.join("manifest.json");
    let alt_manifest_path = path.join("dist").join("manifest.json");
    let actual = if manifest_path.exists() {
        manifest_path
    } else if alt_manifest_path.exists() {
        alt_manifest_path
    } else {
        return Err(AppError::NotFound(format!(
            "No manifest.json found in {:?} (looked at the folder root and dist/)",
            path
        )));
    };

    let manifest = read_manifest(&actual)?;
    let compatibility = validate_compatibility(&manifest);
    Ok(FolderInspection {
        manifest,
        compatibility,
    })
...
mod manifest_schema_tests {
    ...
    #[test]
    fn ignores_legacy_top_level_default_view_field() {
        let json = r#"{
        ...
        // Unknown top-level fields are tolerated so manifests written by
        // other SDK versions (e.g. carrying `tools`) still load.
        parse(json).expect("legacy defaultView should be ignored, not rejected");
    }

    #[test]
    fn ignores_legacy_top_level_main_field() {
        let json = r#"{
        ...
        parse(json).expect("legacy main should be ignored, not rejected");
    }
    ...
    fn tolerates_unknown_top_level_field() {
        ...
        // Installed extensions may carry fields this build no longer knows
        // (e.g. `tools` from the AI era) — they must load, not fail.
        parse(json).expect("unknown top-level fields must be tolerated");
    }
...
mod inspect_folder_tests {
    ...
    fn write_minimal_manifest(dir: &Path) {
        ...
        fs::write(
            ...
            r#"{
                "id": "org.test.folder-ext",
                "name": "Folder Ext",
                "version": "1.0.0",
                "description": "test",
                "type": "extension",
                "background": {"main": "dist/worker.js"}, "commands": [{"id": "ping", "name": "Ping", "mode": "background"}]
            }"#,
        )
        .unwrap();
    }
    ...
    #[test]
    fn inspect_reads_manifest_at_folder_root() {
        let dir = std::env::temp_dir().join(format!("asyar-inspect-root-{}", std::process::id()));
        write_minimal_manifest(&dir);
        let result = inspect_folder(&dir).unwrap();
        assert_eq!(result.manifest.id, "org.test.folder-ext");
        fs::remove_dir_all(&dir).unwrap();
    }

    #[test]
    fn inspect_reads_manifest_under_dist() {
        let dir = std::env::temp_dir().join(format!("asyar-inspect-dist-{}", std::process::id()));
        fs::create_dir_all(dir.join("dist")).unwrap();
        fs::write(
            dir.join("dist").join("manifest.json"),
            r#"{
                "id": "org.test.dist-ext",
                "name": "Dist Ext",
                "version": "0.1.0",
                "description": "test",
                "type": "extension",
        ...
        .unwrap();
        let result = inspect_folder(&dir).unwrap();
        assert_eq!(result.manifest.id, "org.test.dist-ext");
        fs::remove_dir_all(&dir).unwrap();
    }

    #[test]
    fn inspect_errors_when_manifest_missing() {
        let dir = std::env::temp_dir().join(format!("asyar-inspect-empty-{}", std::process::id()));
        fs::create_dir_all(&dir).unwrap();
        let err = inspect_folder(&dir).unwrap_err();
        assert!(err.to_string().contains("manifest.json"));
        fs::remove_dir_all(&dir).unwrap();
    }

    #[test]
    fn inspect_errors_on_invalid_manifest() {
        let dir = std::env::temp_dir().join(format!("asyar-inspect-bad-{}", std::process::id()));
        fs::create_dir_all(&dir).unwrap();
        fs::write(dir.join("manifest.json"), "{ not json").unwrap();
        assert!(inspect_folder(&dir).is_err());
        fs::remove_dir_all(&dir).unwrap();
    }
}
