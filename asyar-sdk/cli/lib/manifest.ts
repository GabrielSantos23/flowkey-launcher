...
export const VALID_PLATFORMS = ['windows'] as const;
...
function validateCommand(cmd: ManifestCommand, i: number, errors: ValidationError[]): void {
  ...
  if (cmd.searchBarPlaceholder && cmd.mode !== undefined && cmd.mode !== 'view') {
    errors.push({
      field: `${base}.searchBarPlaceholder`,
      message: `searchBarPlaceholder is only valid on mode="view" commands (got mode="${cmd.mode}")`,
    });
  }
