...
describe('link command', () => {
  ...
  it('registers extension in dev_extensions.json atomically', async () => {
    vi.mocked(fs.existsSync).mockImplementation((p) => {
      if (norm(p) === '/tmp/asyar/dev_extensions.json') return true;
      return false;
    });
    vi.mocked(fs.readFileSync).mockReturnValue(
    ...
    const devExtWrite = writeCalls.find((call) =>
      norm(call[0]).startsWith('/tmp/asyar/dev_extensions.json.tmp'),
    );
    ...
    expect(norm(vi.mocked(fs.renameSync).mock.calls[0]?.[1])).toBe('/tmp/asyar/dev_extensions.json');
  });
...
describe('unlink command', () => {
  ...
  it('unlinks extension directory and removes entry from dev_extensions.json', async () => {
    vi.mocked(fs.existsSync).mockImplementation((p) => {
      if (norm(p) === '/tmp/asyar/extensions/com.test.worker-only') return true;
      if (norm(p) === '/tmp/asyar/dev_extensions.json') return true;
      return false;
    });
    ...
    expect(norm(vi.mocked(fs.unlinkSync).mock.calls[0]?.[0])).toBe(
      '/tmp/asyar/extensions/com.test.worker-only',
    );
    expect(fs.writeFileSync).toHaveBeenCalled();
    const writeCalls = vi.mocked(fs.writeFileSync).mock.calls;
    const devExtWrite = writeCalls.find((call) =>
  ...
  it('handles non-symlink target directory with rmSync', async () => {
    ...
    expect(norm(vi.mocked(fs.rmSync).mock.calls[0]?.[0])).toBe(
      '/tmp/asyar/extensions/com.test.worker-only',
    );
    expect(vi.mocked(fs.rmSync).mock.calls[0]?.[1]).toEqual({
      recursive: true,
      force: true,
    });
  });
  ...
  it('unlinks from dev flavor when --dev is provided', async () => {
    vi.mocked(fs.existsSync).mockImplementation((p) => {
      if (norm(p) === '/tmp/asyar-dev/extensions/com.test.worker-only') return true;
      if (norm(p) === '/tmp/asyar-dev/dev_extensions.json') return true;
      return false;
    });
    vi.mocked(fs.lstatSync).mockReturnValue({
      isSymbolicLink: () => true,
    ...
    expect(vi.mocked(getDevExtensionsFile)).toHaveBeenCalledWith(true);
    expect(norm(vi.mocked(fs.unlinkSync).mock.calls[0]?.[0])).toBe(
      '/tmp/asyar-dev/extensions/com.test.worker-only',
    );
    const writeCalls = vi.mocked(fs.writeFileSync).mock.calls;
    const devExtWrite = writeCalls.find((call) =>
      norm(call[0]).startsWith('/tmp/asyar-dev/dev_extensions.json.tmp'),
    );
    expect(devExtWrite).toBeDefined();
